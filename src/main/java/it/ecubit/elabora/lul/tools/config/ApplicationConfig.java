...
public class ApplicationConfig implements CommandLineRunner {
	...
	public void run(String... args) {
		...
		if(numArgs < 1) {
			...
			log.error("Usage: java -jar <ELABORA TOOLS JAR> <PDF REPORT FILENAME> (-o <OUTPUT DIR>). The -o argument is optional and defaults to the directory in which PDF resides.");
		}
		try {
			...
			for(String page : pages) {
				try {
					...
					String employeeNameSurname = monthlyReport.getEmployee().getName() + " " + monthlyReport.getEmployee().getSurname();
					String englishMonthName = StringUtils.capitalize(monthlyReport.getMonth().getName().toLowerCase());
					log.info(String.format("Loaded %s monthly report for employee %s from page %d", englishMonthName, employeeNameSurname, pageIndex));
			...
			log.info(String.format("Exporting data to %s Excel LUL template in dir %s", excelFilename, exportDirPath.toString()));
